...
Prebuilt `acac` binary for **darwin-arm64** (Apple Silicon macOS).
...
- Built on GitHub Actions from the [acac-cli](https://github.com/RyosukeDTomita/acac-cli) source, on a native macOS runner with GHC 9.12.2 + cabal (linked against the OS's standard system libraries).
